...
class M_Pengabdian extends CI_Model {
	...
	public function getFileNameLaporanKemajuan($id)
    {
        ...
        $this->db->from('tbl_pengabdian'); // Ganti "nama_tabel_pengabdian" dengan nama tabel pengabdian yang sesuai
	...
	public function getFileName($id_pengabdian)
    {
	...
	public function getFileNameFile($id)
    {
        ...
        $this->db->from('tbl_dokumen_logbook'); // Ganti "nama_tabel_pengabdian" dengan nama tabel pengabdian yang sesuai
	...
	public function getFileNameLpKemajuan($id)
    {
        ...
        $this->db->from('tbl_dok_lp_kemajuan'); // Ganti "nama_tabel_pengabdian" dengan nama tabel pengabdian yang sesuai
    ...
    {
        $this->db->select('url_file');
        $this->db->from('tbl_dok_lp_kemajuan_akhir'); // Ganti "nama_tabel_pengabdian" dengan nama tabel pengabdian yang sesuai
        $this->db->where('luaran_hasil_id', $id);
        $query = $this->db->get();
	...
	public function create_pengabdian($data=[])
	{
		return $this->db->insert("tbl_pengabdian",$data);
	}

	public function delete_luaran_by_ids($luaranIds, $pengabdianId)
    {
        $this->db->where('pengabdian_id', $pengabdianId);
        $this->db->where_in('luaran_wajib_id', $luaranIds);
        $this->db->delete('tbl_luaran_hasil');
    }

	public function get_existing_luaran_wajib_ids($pengabdianId)
    {
	...
	public function delete_luaran_tambahan_by_ids($luaranIds, $pengabdianId)
    {
        $this->db->where('pengabdian_id', $pengabdianId);
	...
	public function get_existing_luaran_tambahan_ids($pengabdianId)
    {
	...
	{
		return $this->db->delete("tbl_pengabdian",["id_pengabdian" => $id]);
	}
	...
	public function get_all_unread_pesan($option=NULL)
	{
		...
		if ($option=="count") {
			return $this->db->get_where("tbl_pengabdian",["status_pesan" => 0])->num_rows();
		}else{
			return $this->db->order_by("judul",'DESC')
						->get("tbl_pengabdian",5)->result_array();
		}
	}
